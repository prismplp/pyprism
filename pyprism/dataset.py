...
def discretize(X, thresh_uniq=10,disc_bins=10):
  ...
  discrete_cols = [col for col in X.columns if X[col].nunique() < thresh_uniq and X[col].dtype != 'object']
  ...
  for col in continuous_cols:
      ...
      if not col_data.empty:
          ...
          discretizer = KBinsDiscretizer(n_bins=disc_bins, encode='ordinal', strategy='quantile')
          ...
          discretized_series = pd.Series(discretized_data.flatten(), index=col_data.index)
...
def discretize_y(y,thresh_uniq=10, disc_bins=10):
  ...
  if y_data.nunique()>thresh_uniq:
    if not y_data.empty:
        # Use KBinsDiscretizer to discretize based on quantiles (percentiles)
        discretizer_y = KBinsDiscretizer(n_bins=disc_bins, encode='ordinal', strategy='quantile')
        # Fit and transform the non-NaN data
        discretized_y_data = discretizer_y.fit_transform(y_data.values.reshape(-1, 1))
        # Create a pandas Series with the same index as the original non-NaN data
        y_discretized = pd.Series(discretized_y_data.flatten(), index=y_data.index)
    else:
        y_discretized = pd.Series(dtype=float) # Create an empty series if y_data was empty
  else:
    return y_data,None
  return y_discretized,discretizer_y

def plot_discretization(X: pd.DataFrame,hist_bins:int =100, disc_bins:int =10,title:str =None):
  # Discretize variables based on percentiles and add vertical lines
  for col in X.columns:
      # Calculate the 10th percentile boundaries
      percentiles = np.percentile(X[col].dropna(), np.arange(0, 101, 100//disc_bins))
...
def to_dat(X_discretized: pd.DataFrame, y_discretized: pd.DataFrame, out_filename: str="output.dat", pred:str ="data"):
  ...
  for index, row in X_discretized.iterrows():
      ...
      y_value = aligned_y.loc[index] if index in aligned_y.index and pd.notna(aligned_y.loc[index]) else "_"
      if y_value != "_":
